describe DateOfBirth do
  ...
  describe '#birthday_is_in_future?' do
    context 'birthday has not passed' do
      it 'returns 1' do
        dob = DateOfBirth.new('1993-12-10')
        expect(dob.birthday_is_in_future?).to eq 1
      end
    end
    context 'birthday is today' do
      it 'returns 0' do
        dob = DateOfBirth.new('1993-12-09')
        expect(dob.birthday_is_in_future?).to eq 0
      end
    end
    context 'birthday has passed' do
      it 'returns 0' do
        dob = DateOfBirth.new('1993-11-10')
        expect(dob.birthday_is_in_future?).to eq 0
      end
    end
    context 'dob is on 2/29 and it is not a leap year' do
      context 'birthday has passed' do
        it 'returns 0' do
          dob = DateOfBirth.new('1988-02-29')
          expect(dob.birthday_is_in_future?).to eq 0
        end
      end
      context 'birthday has not passed' do
        it 'returns 1' do
          dob = DateOfBirth.new('1988-12-29')
          expect(dob.birthday_is_in_future?).to eq 1
        end
      end
    end
  ...
  describe '#datify' do
    ...
    context 'dob is a String object' do
      it 'parses the dob into a Time object and returns it' do
        ...
        dob = DateOfBirth.new('1988-12-30')
